...
export function RepoInputPage() {
  ...
  const { repoId, status, setStatus, setRepoId } = useRepoContext()
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (sourceType === 'github') {
        result = await ingestRepository({ source_type: 'github', source: repoUrl })
      } else {
        result = await ingestRepository({ source_type: 'zip', source: '', file: zipFile! })
      }
