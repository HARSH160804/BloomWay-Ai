...
BloomWay AI is an AI-powered codebase understanding tool that enables developers to quickly comprehend unfamiliar codebases through intelligent analysis, visualization, and interactive chat. The system ingests GitHub repositories, generates architecture summaries with visual diagrams, provides multi-level code explanations, and supports conversational queries using retrieval-augmented generation (RAG).

## Glossary

- **BloomWay_System**: The complete AI-powered codebase understanding application
...
1. WHEN a user begins analyzing a repository, THE BloomWay_System SHALL create a new session in the DynamoDB Sessions table
2. WHEN a session is created, THE BloomWay_System SHALL assign a unique session identifier
3. WHEN a session is created, THE BloomWay_System SHALL set a time-to-live (TTL) of 24 hours
4. WHEN the TTL expires, THE BloomWay_System SHALL automatically delete the session and associated data
5. WHEN a session is deleted, THE BloomWay_System SHALL remove all associated embeddings from the Vector_Store
6. WHEN a session is deleted, THE BloomWay_System SHALL remove all associated artifacts from the S3 bucket
7. WHEN a user returns to an active session, THE BloomWay_System SHALL restore the session state including all analyzed repositories
...
6. WHEN data is written to DynamoDB, THE BloomWay_System SHALL handle write capacity errors gracefully
7. WHEN data is written to S3, THE BloomWay_System SHALL use server-side encryption
...
2. WHEN a Lambda function times out, THE BloomWay_System SHALL return an error message indicating the operation exceeded time limits
3. WHEN the LLM service is unavailable, THE BloomWay_System SHALL return an error message indicating the AI service is temporarily unavailable
...
5. WHEN DynamoDB or S3 operations fail, THE BloomWay_System SHALL log the error and return a user-friendly error message
6. THE BloomWay_System SHALL implement retry logic with exponential backoff for transient AWS service failures
7. THE BloomWay_System SHALL validate all user inputs and return descriptive validation errors
...
5. WHEN a session expires, THE BloomWay_System SHALL ensure all associated data is permanently deleted
6. THE BloomWay_System SHALL not store GitHub credentials or access tokens beyond the ingestion request lifecycle
